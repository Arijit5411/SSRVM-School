...
const Awards_And_Achievements = ({ seodata, awardsData }) => {
    ...
    const filteredAwards = awardsAndAchievements.filter(
        award => award.attributes.year === selectedYear && award.attributes.award_type === selectedOption
    );
        ...
        return (
            <div className='row'>
                {currentAwards.map((award, index) => (
                    <div className='col-lg-3' key={index} onClick={() => handleImageClick(`${siteUrl}${award.attributes.image.data.attributes.url}`)}>
                        <div className="card wrap-news">
                            <img src={`${siteUrl}${award?.attributes?.image?.data?.attributes?.url}`} className="wrap-img-top1 wrap-side-award" alt="..." />
                ...
                <div className='desktophide'>
                    <section>
                        <div className="container">
                            <div className="wrap-item-text1 news-item">
                                <h1 className="wrap-award-heading">
                                    Awards and Achievements
                                </h1>
                                <div className="award-dropdown">
                                    <div>
                                        <select value={selectedOption} onChange={handleChange} className="drop">
                                            <option value="school award">School Awards</option>
                                            <option value="individual award">Individual Awards</option>
                                        </select>
                                    </div>

                                    <div>
                                        <select value={selectedYear} onChange={handleYearChange} className="drop">
                                            {Array.from(new Set(awardsAndAchievements.map(award => award.attributes.year)))
                    ...
                    <section className="container">
                        ...
                        <div className="pagination-blog">
                            ...
                            {Array.from({ length: Math.ceil(filteredAwards.length / awardsPerPage) }, (_, index) => (
                                <button key={index} onClick={() => paginate(index + 1)} className={currentPage === index + 1 ? "active" : ""}>
                                    {index + 1}
                                </button>
                            ))}

                            {currentPage < Math.ceil(filteredAwards.length / awardsPerPage) && (
