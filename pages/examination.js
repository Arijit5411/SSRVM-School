...
export const getStaticProps = async () => {
    ...
    const res1 = await fetch(`${siteUrl}/api/examinations?populate=*`)
...
const Examination = ({ seodata, optionData }) => {
    ...
    useEffect(() => {
        ...
        //             const seoAttributes = data.data[32].attributes;
        //             setSeoData({
        ...
        if (seodata && seodata?.data && seodata?.data?.length > 0) {
            const seoAttributes = seodata.data[32].attributes;
            setSeoData({
                title: seoAttributes.title || '',
                metaTitle: seoAttributes.metaTitle || '',
                metaDescription: seoAttributes.metaDescription || '',
                                ...
                                <div className="col-lg-6 wrap-schedule">
                                    <div className="sche-item">
                                        <a href={exam_midterm_pdf} download>{exam_midterm}
                                            {/* Exam Schedule for year Mid Term 2021-22 */}
                                        </a>
                                    </div>
                                </div>
                                <div className="col-lg-6 wrap-schedule">
                                    <div className="sche-item">
                                        <a href={exam_unit_pdf} download>{exam_unit}
                                            {/* Exam Schedule for year Unit 2021-22 */}
                                        </a>
                                    </div>
                                </div>
                                <div className="col-lg-6 wrap-schedule">
                                    <div className="sche-item">
                                        <a href={exam_fullterm_pdf} download>{exam_fullterm}
                                            {/* Exam Schedule for year Full Term 2021-22 */}
                                        </a>
                                    </div>
                                </div>
                            </div>
