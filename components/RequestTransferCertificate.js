...
const RequestTransferCertificate = () => {
  ...
  const [classType, setclassData] = useState([]);
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      try {
        const response = await fetch(
          `${siteUrl}/api/request-transfer-certificates`,
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          ...
          await fetch(`/api/requestTransferCertMail`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${siteUrl}/api/transfer-certificate?populate=*`
        );
        const data = await response.json();
        console.log("year", data);
        setAcademicYears(data.data.attributes);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchClassData = async () => {
      try {
        const response = await fetch(
          `${siteUrl}/api/school-total-classes?populate=*`
        );
        const data = await response.json();
              ...
              <div className="row">
                ...
                <div className="col-sm-6">
                  ...
                  {formErrors.registrationId && (
                    <span className="error">{formErrors.registrationId}</span>
                  )}
                </div>
                ...
                <div className="col-sm-6">
                  ...
                  >
                    ...
                    {academicYear.Academic_Years?.map((option) => (
                      <option key={option} value={option.Year}>
                        {option.Year}
                      </option>
                    ))}
                  </select>
                ...
                <div className="col-sm-6">
                  ...
                  >
                    ...
                    {classType.map((option) => (
                      <option key={option} value={option.attributes.name}>
